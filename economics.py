...
def biased_wealth_transfer(agent_i, agent_j, beta=0.05, p_base=0.1, gamma=0.3):
    if agent_i.wealth == 0 or agent_j.wealth == 0:
        return "No transfer: one agent has zero wealth"
    ...
    return f"{donor.unique_id} → {recipient.unique_id}: transferred {delta_w:.2f}"


def crime_propensity(wealth, w_c=100, delta=0.05, zeta=2.0, epsilon=1e-2):
    """
    Calculate an agent's probability of committing a crime based on their current wealth.
